...
func ShowSetting() {
	...
	topLeft2Box, _ := gtk.BoxNew(gtk.ORIENTATION_VERTICAL, 10)
	checkButton1Box, _ := gtk.BoxNew(gtk.ORIENTATION_HORIZONTAL, 10)
	checkButton2Box, _ := gtk.BoxNew(gtk.ORIENTATION_HORIZONTAL, 10)
	randomCheckButton, _ := gtk.CheckButtonNewWithLabel("随机消息")
	randomCheckButton.SetHExpand(true)
	withoutStopCheckButton, _ = gtk.CheckButtonNewWithLabel("连续不停")
	withoutStopCheckButton.SetHExpand(true)
	averageCheckButton, _ := gtk.CheckButtonNewWithLabel("间隔平均")
	averageCheckButton.SetHExpand(true)
	beforeCheckButton, _ = gtk.CheckButtonNewWithLabel("前置按键")
	beforeCheckButton.SetHExpand(true)
